...
*version alpha 0.8_alpha:
* added the support for the Sensor BMA280 (Acceleration) and BME280 (Ambient Pressure, Humidity, Temperature)
* added Support for the Sensirion SDP610-125Pa
*/
...
String version ="Due 0.8_alpha";	//version string
...
// ## Sensors ##
static int CS_BMa280 = 10; //Acceleration Sensor
static int CS_BMe280 = 4; //Environment Sensor: Pressure, Temperatur, Humidity
static int INT_1 = 14; //interrupt recognition pin (can be every digital Input)
int DiffPressSensAdress = 0x40;  //Adress for SDP610-125 Sensor
...
Adafruit_BME280_Due bme(CS_BMe280, 12); // Environment Sensor for Ambient Pressure, Humidity and Temperature
BMA280 BMA280(CS_BMa280,INT_1);  // Acceleration Sensor
SDP610_Lib pDiff(DiffPressSensAdress);   /// Differetntial Pressure Sensor
...
int32_t P_Angle2Step(int32_t _gamma){
	...
	//const float _r		= 1.0;			//[mm] radius of bolt //unused!!!
...
void printHelp(){
  ...
  Serial.println("A0 : Stops datastream");
...
void setup(){
  ...
  analogWriteResolution(ADCbits);
	...
	dac.begin();
	L_setLoad(0);	
  // Start Acvceleration Sensor
  BMA280.begin();
  // Start Environment Sensor
  pDiff.begin();  //Start Differntial Pressure Sensor
...
void loop(){
	...
	if (sendData){
      ...
      BMA280.readSensor(accValue);
			...
			long	temperature			= (long)(bme.readTemperature()*10);// 23*10;				// temperature in deg C*10
			long pressure_amb	= (long)bme.readPressure() ;// 101300;			// ambient pressure in Pa
			long humidity			= (long)(bme.readHumidity()*10);					// humidity in %*10
      long windVelocity   = pDiff.velocity((float)pressure_amb, (float)(temperature/10))*100;    //Windspeed in cm/s  
      //WindSpeed;      // send back the set wind speed for the moment
			int32_t g_x					= accValue[0];					// Acceleration against wind flow direction in mg
			int32_t g_y					= accValue[1];					// Acceleration perpendicular to wind flow in horizontal direction in mg
			int32_t g_z					= accValue[2];					// Acceleration in up direction  in mg
			...
			Serial.println("{");
				...
				P_Step();
				Serial.print(constStringLength(pressure_amb,6));
				P_Step();
				Serial.println(",");
